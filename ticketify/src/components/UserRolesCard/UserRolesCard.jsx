...
const UserRolesCard = ({ roles = [], userRoles = [] }) => {
    ...
    return (
        <div className="grid md:grid-cols-4 2xl:grid-cols-2 place-items-center">
            {
                userRoles.map((userRole) => {
                    if (userRole.status)
                        if (previousUser != userRole.userName) {
                            ...
                            return (
                                <div key={userRole.userId} className='w-full max-w-4xl 2xl:max-w-5xl 2xl:text-2xl p-6 mb-6 bg-white border border-gray-200 rounded-lg shadow-md hover:shadow-xl font-montserrat'>
